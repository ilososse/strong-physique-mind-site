...
// 🌐 Translation table FR ↔ EN to merge data
...
// Normalize exercise name to canonical form (English)
...
async function getAuthedUserOrWarn(infoEl) {
  ...
  if (error) {
    ...
    infoEl && (infoEl.textContent = "❌ Auth error (see console).");
    return null;
  }
  if (!data?.user) {
    infoEl && (infoEl.textContent = "⚠️ Log in to save and view your sessions.");
    return null;
  }
  return data.user;
}
...
function clearPrivateUI() {
  ...
  if (hist) hist.innerHTML = "<p style='font-size:12px; color:#777;'>Log in to see your sessions.</p>";
  ...
  if (dashInfo) dashInfo.textContent = "Log in to view your dashboard.";
...
function updateAuthUI(user) {
  ...
  if (user) {
    ...
    if (emailInput) {
      ...
      emailInput.title = "Logged in";
    }

    if (passInput) {
      passInput.disabled = true;
      passInput.value = "••••••••";
      passInput.title = "Password hidden";
    }
  ...
  tr.innerHTML = `
    <td class="ex-input-container">
      <input
        type="text"
        class="ex-name"
        placeholder="Pull-ups, Front Lever..."
        autocomplete="off"
        value="${data.name ?? ""}"
      />
    </td>
    ...
    <td>
      <input
        type="text"
        class="ex-ressenti"
        placeholder="Overall feeling"
        value="${data.ressenti ?? ""}"
      >
    </td>

    <td class="add-set-cell" style="display: none;">
      <button class="add-set-btn" type="button" title="Add a set">+</button>
    </td>
...
if (!mode) {
  // fallback for unknown/custom exercise → reps by default
  mode = "reps";
}
      ...
      html += `
        <div class="session-card">
          <div class="session-card-header">
            <span class="label">Session of ${escapeHtml(dateStr)}</span>
...
async function deleteSessionByDate(date) {
  ...
  const ok = confirm(`Delete ALL sessions for ${date}?`);
  if (!ok) return;
  ...
  info.textContent = "✅ Sessions for the day deleted";
  scheduleUIRefresh({ forceSessions: true, forceFeed: true });
}
...
function updateMetricOptions() {
  ...
  // Use dataset to determine exercise type
  if (exerciseInput.dataset.isHold === "true") {
    options = [
      { value: "avg_hold", label: "Mean holds" },
      { value: "max_hold", label: "Max holds" },
      { value: "count_hold", label: "Number of holds" },
      { value: "sum_hold", label: "Sum of holds" }
    ];
  } else {
    options = [
      { value: "avg_reps", label: "Mean reps" },
      { value: "max_rep", label: "Max reps" },
      { value: "count_sets", label: "Number of sets" },
      { value: "sum_reps", label: "Sum of reps" }
    ];
  }
...
async function buildChartData(exerciseName, metricKey) {
  ...
  // Normalize searched exercise name
  ...
  // 2.5) Detect exercise mode from sessions
  let exerciseMode = "reps"; // default
  Array.from(variantMap.values()).some(variant => {
    // Search for mode in sessions
    sessions.some(s => {
...
async function renderExerciseChart() {
  ...
  const user = sessData?.session?.user || null;
  if (!user) {
    info.textContent = "Log in to view your dashboard.";
  ...
  if (!exerciseName) {
    info.textContent = "Choose an exercise or enter a name for a custom exercise.";
    if (exerciseChart) { exerciseChart.destroy(); exerciseChart = null; }
    return;
  }
  ...
  if (!labels.length || !datasets.length) {
    info.textContent = "No data found for this exercise and metric.";
    if (exerciseChart) { exerciseChart.destroy(); exerciseChart = null; }
    return;
  }
...
async function initDashboard() {
  ...
  if (savedExercise) {
    ...
    // Restore mode to display correct metrics
    if (savedMode) {
  ...
  exerciseInput.addEventListener("input", async () => {
    ...
    results.forEach(ex => {
      ...
      li.addEventListener("click", async () => {
        ...
        dropdown.style.display = "none";
        // Save mode for restoration on reload
...
async function initUsernameUI() {
  ...
  // Default state: locked and hidden
  input.disabled = true;
  input.value = "";
